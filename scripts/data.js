...
Array.from(document.querySelector(".ExamsColumn").children).forEach(RowItem => {
    RowItem.querySelectorAll("time").forEach(Time => {
        ...
        const Timestamp = Math.floor(TimeDate / 1000);

        if (Math.floor(new Date() / 1000) <= Timestamp) {
            ClassData.Exams.forEach(Exam => {
                console.log(Exam);
                console.log(typeof Exam.Timestamp);
                console.log(Exam.Timestamp, Timestamp);
                console.log(Exam.Timestamp === Timestamp);
                if (Exam.Timestamp === Timestamp) {
                    console.log(Exam);
                }
            });
        }
